...
`apps/what-would-win/`. Set `WHAT_WOULD_WIN_ROOT` to use another source checkout. Treat the copied route as a release artifact: update it through the sync command, not by hand. The verifier requires the app entry point, hashed assets, manifest, icons, social card, and public `legal-notices.txt` generated by the source repository's release audit.
