...
async function fixture(version = "0.3.0-alpha.97") {
  ...
  const files = new Map([
    ...
    ["src.0123456789abcdefabcd/game.js", Buffer.from("export const alpha = 97;\n")],
...
test("Alpha.97 Briarhold package validation authenticates exact inventory and hosted bytes", async t => {
  ...
  assert.equal(validated.manifest.version, "0.3.0-alpha.97");
...
test("Briarhold sync validation rejects an older package and unmanifested files", async t => {
  const old = await fixture("0.3.0-alpha.96");
  const extra = await fixture();
  t.after(() => Promise.all([fs.rm(old, {recursive: true}), fs.rm(extra, {recursive: true})]));
  await assert.rejects(validateBriarholdPackage(old), /Expected Briarhold 0\.3\.0-alpha\.97/u);
...
test("Briarhold landing and catalogue fallbacks expose browser Alpha.97 and experimental co-op", async () => {
  ...
  assert.match(page, /Android APK · Alpha\.97/u);
