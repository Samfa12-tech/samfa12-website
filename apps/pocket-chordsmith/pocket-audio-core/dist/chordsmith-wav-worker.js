(() => {
  ...
  function chordsmithBassIndexToMidi({ key = "C", scale = "major", noteIndex = 0 } = {}) {
  ...
  function normaliseLofiTexture(value = {}, preset = getLofiStylePreset()) {
    ...
    return {
      ...
      vinylCrackle: clamp013(source.vinylCrackle ?? presetTexture.vinylCrackle ?? DEFAULT_LOFI_TEXTURE.vinylCrackle),
      tapeHiss: clamp013(source.tapeHiss ?? presetTexture.tapeHiss ?? DEFAULT_LOFI_TEXTURE.tapeHiss),
      wowFlutter: clamp013(source.wowFlutter ?? presetTexture.wowFlutter ?? DEFAULT_LOFI_TEXTURE.wowFlutter),
      warmth: clamp013(source.warmth ?? presetTexture.warmth ?? DEFAULT_LOFI_TEXTURE.warmth),
      lowPassAge: clamp013(source.lowPassAge ?? presetTexture.lowPassAge ?? DEFAULT_LOFI_TEXTURE.lowPassAge),
      bitCrush: clamp013(source.bitCrush ?? presetTexture.bitCrush ?? DEFAULT_LOFI_TEXTURE.bitCrush)
    };
  }
  ...
  function normaliseChipTexture(value = {}, preset = getChipStylePreset()) {
    ...
    return {
      ...
      bitDepth: clamp013(source.bitDepth ?? presetTexture.bitDepth ?? DEFAULT_CHIP_TEXTURE.bitDepth),
      sampleRateCrush: clamp013(source.sampleRateCrush ?? presetTexture.sampleRateCrush ?? DEFAULT_CHIP_TEXTURE.sampleRateCrush),
      pulseWidth: clamp013(source.pulseWidth ?? presetTexture.pulseWidth ?? DEFAULT_CHIP_TEXTURE.pulseWidth),
      pitchDrift: clamp013(source.pitchDrift ?? presetTexture.pitchDrift ?? DEFAULT_CHIP_TEXTURE.pitchDrift),
      saturation: clamp013(source.saturation ?? presetTexture.saturation ?? DEFAULT_CHIP_TEXTURE.saturation),
      stereoSpread: clamp013(source.stereoSpread ?? presetTexture.stereoSpread ?? DEFAULT_CHIP_TEXTURE.stereoSpread)
    };
  }
  ...
  function normaliseMetalTexture(value = {}, preset = getMetalStylePreset()) {
    ...
    return {
      ...
      drive: clamp013(source.drive ?? presetTexture.drive ?? DEFAULT_METAL_TEXTURE.drive),
      palmMute: clamp013(source.palmMute ?? presetTexture.palmMute ?? DEFAULT_METAL_TEXTURE.palmMute),
      lowTightness: clamp013(source.lowTightness ?? presetTexture.lowTightness ?? DEFAULT_METAL_TEXTURE.lowTightness),
      presence: clamp013(source.presence ?? presetTexture.presence ?? DEFAULT_METAL_TEXTURE.presence),
      roomSize: clamp013(source.roomSize ?? presetTexture.roomSize ?? DEFAULT_METAL_TEXTURE.roomSize),
      pickAttack: clamp013(source.pickAttack ?? presetTexture.pickAttack ?? DEFAULT_METAL_TEXTURE.pickAttack)
    };
  }
  ...
  function normaliseFunkParameters(value = {}, preset = getFunkStylePreset()) {
    const source = value && typeof value === "object" && !Array.isArray(value) ? value : {};
    return Object.fromEntries(Object.keys(DEFAULT_FUNK_PARAMETERS).map((key) => [key, clamp013(source[key] ?? preset.parameters[key] ?? DEFAULT_FUNK_PARAMETERS[key])]));
  }
  ...
  function normaliseWesternProjectSettings(project = {}) {
    ...
    return {
      ...
      parameters: Object.fromEntries(Object.keys(DEFAULT_WESTERN_PARAMETERS).map((key) => [key, clamp013(source[key] ?? preset.parameters[key] ?? DEFAULT_WESTERN_PARAMETERS[key])]))
    };
  }
  ...
  function normalisePocketChordsmithProject(raw, options = {}) {
    ...
    return {
      ...
      meta: {
        ...
        bpm: clamp2(asInt(project.bpm, DEFAULT_BPM), 40, 240),
        timeSig,
        resolution,
        swing: clamp2(asNumber(project.swing, 0), 0, 0.35),
        ppq: DEFAULT_PPQ,
      ...
      mixer: {
        masterVolume: clamp2(asNumber(project.masterVolume ?? project.masterVol, DEFAULT_MASTER_VOLUME), 0, 1),
  ...
  function normaliseSection(project, id, context) {
    ...
    const bars = clamp2(asInt(sectionSource.bars, context.sectionBars[id]), 1, 16);
    ...
    return {
      ...
      bars,
      active,
      progression: fitArray(progressionRaw || DEFAULT_PROGRESSION, Math.max(1, context.sectionBars[id]), 0, (value) => clamp2(asInt(value, 0), 0, 6)),
      drums: {
      ...
      chords: {
        ...
        octave: clamp2(asInt(project.chordOctave, 0), -2, 2)
      },
      melody: melodyTracks.map((notes, index) => ({
        ...
        octave: clamp2(asInt((project[`melodyOctaves${id}`] || [])[index], 0), -2, 2),
        mute: Boolean((project[`melodyMute${id}`] || [])[index]),
        solo: Boolean((project[`melodySolo${id}`] || [])[index]),
        pan: clamp2(asNumber((project[`melodyPan${id}`] || [])[index], 0), -1, 1),
      ...
      guitar: {
        ...
        volume: clamp2(asNumber(project.guitarVolume, DEFAULT_STEM_MIX.guitar.volume), 0, 1),
  ...
  function normaliseRichEvent(value, context) {
    const source = value && typeof value === "object" && !Array.isArray(value) ? value : {};
    const step = source.step === void 0 ? void 0 : clamp2(asNumber(source.step, 0), 0, Math.max(0, context.steps - 1e-6));
    ...
    return {
      ...
      ...notes ? { notes } : {},
      velocity: clamp2(asNumber(source.velocity, 100), 0, 127),
  ...
  function normaliseStemMix(project) {
    ...
    out.drums.volume = clamp2(asNumber(project.beatVolume ?? project.beatVol, out.drums.volume), 0, 1);
    out.bass.volume = clamp2(asNumber(project.beatVolume ?? project.beatVol, out.bass.volume), 0, 1);
    out.chords.volume = clamp2(asNumber(project.chordVolume ?? project.chordVol, out.chords.volume), 0, 1);
    out.melody.volume = clamp2(asNumber(project.leadVolume ?? project.leadVol, out.melody.volume), 0, 1);
    out.guitar.volume = clamp2(asNumber(project.guitarVolume, out.guitar.volume), 0, 1);
    STEM_IDS.forEach((id) => {
  ...
  function normaliseFx(project) {
    ...
    return {
      ...
      delay: clamp2(asNumber(project.fxDelay, DEFAULT_FX.delay), 0, 1),
      chorus: clamp2(asNumber(project.fxChorus, DEFAULT_FX.chorus), 0, 1),
      flanger: clamp2(asNumber(project.fxFlanger, DEFAULT_FX.flanger), 0, 1),
      reverb: clamp2(asNumber(project.fxReverb, DEFAULT_FX.reverb), 0, 1),
      mix: clamp2(asNumber(project.fxMix, DEFAULT_FX.mix), 0, 1),
      sidechain: {
        enabled: Boolean(project.sidechainOn ?? project.pumpChordsEnabled),
        amount: clamp2(asNumber(project.sidechainAmount ?? project.pumpAmount, DEFAULT_FX.sidechain.amount), 0, 1)
      },
  ...
  function normaliseSectionBars(value) {
    const out = {};
    SECTION_IDS.forEach((id) => {
      out[id] = clamp2(asInt(value?.[id], 4), 1, 16);
    });
    return out;
  }
  ...
  function progressionDiffers(value) {
    ...
    return value.some((item, index) => clamp2(asInt(item, DEFAULT_PROGRESSION[index] ?? 0), 0, 6) !== (DEFAULT_PROGRESSION[index] ?? 0));
  }
  function normaliseBeat(value) {
    return clamp2(asInt(value, 0), 0, 2);
  }
  function normaliseMaybeNote(value, max) {
    if (value === null || value === void 0 || value === "") return null;
    const note = asInt(value, -1);
    return note < 0 ? null : clamp2(note, 0, max);
  }
  ...
  var PocketAudio = class {
    constructor(options = {}) {
      ...
      this.intensity = clamp3(Number(options.intensity ?? 0), 0, 1);
    ...
    setIntensity(value) {
      this.intensity = clamp3(Number(value), 0, 1);
    ...
    duck(enabled = true, options = {}) {
      this.ducking = {
        ...
        amount: clamp3(Number(options.amount ?? (enabled ? 0.45 : 0)), 0, 1),
    ...
    lowpass(amount = 1) {
      this.lowpassAmount = clamp3(Number(amount), 0, 1);
    ...
    setStemVolume(stem, volume) {
      this.patchStem(stem, { volume: clamp3(Number(volume), 0, 1) });
    }
  ...
  function scheduleSimpleAudioEvent(context, event, project, targetAudioTime) {
    ...
    gain.gain.exponentialRampToValueAtTime(1e-4, start + Math.max(0.04, (event.duration || 0.08) * voice.durationScale));
  ...
  function scheduleBassLayer(context, start, midi, duration, wave, volume, cutoff, attack, options = {}) {
    const gain = context.createGain();
    gain.gain.setValueAtTime(1e-4, start);
    gain.gain.linearRampToValueAtTime(Math.max(1e-4, volume), start + Math.max(2e-3, Number(attack || 0.01)));
    gain.gain.exponentialRampToValueAtTime(1e-4, start + Math.max(0.04, duration));
    const filter = context.createBiquadFilter();
    filter.type = "lowpass";
  ...
  function liveFunkParameters(event) {
    const source = event.soundProfile?.parameters || {};
    const clamp015 = (value, fallback) => Math.max(0, Math.min(1, Number(value ?? fallback)));
    return {
      pocket: clamp015(source.pocket, 0.72),
      ghostNotes: clamp015(source.ghostNotes, 0.42),
      slapAmount: clamp015(source.slapAmount, 0.68),
      popBrightness: clamp015(source.popBrightness, 0.62),
      muteDepth: clamp015(source.muteDepth, 0.74),
      stabTightness: clamp015(source.stabTightness, 0.76)
    };
  }
  function scheduleMetalGuitarAudioEvent(context, event, project, start) {
    ...
    const technique = event.technique?.metal || {};
    const palmMute = clamp3(technique.palmMute ?? (event.articulation === "palm_mute" || event.articulation === "chug" ? texture.palmMute : Number(texture.palmMute || 0) * 0.3), 0, 1);
    const drive = 1.4 + clamp3(Number(texture.drive || 0), 0, 1) * 8.2;
    const tightness = clamp3(Number(texture.lowTightness || 0), 0, 1);
    const presence = clamp3(Number(texture.presence || 0), 0, 1);
    const roomSize = clamp3(Number(texture.roomSize || 0), 0, 1);
    const pickAttack = clamp3(Number(texture.pickAttack || 0), 0, 1);
  ...
  function scheduleTransientOscillator(context, start, midi, duration, volume) {
    ...
    const gain = context.createGain();
    const osc = context.createOscillator();
    gain.gain.setValueAtTime(Math.max(1e-4, volume), start);
  ...
  // src/export/chordsmith-wav-worker.js
  async function renderChordsmithWavWorkerJob({ project, options }) {
    ...
    return {
      bytes: wavBytes.byteOffset === 0 && wavBytes.byteLength === wavBytes.buffer.byteLength ? wavBytes.buffer : wavBytes.buffer.slice(wavBytes.byteOffset, wavBytes.byteOffset + wavBytes.byteLength),
      type: "audio/wav"
    };
  }
